    ...
    <div className="min-h-screen flex items-center justify-center bg-[#0a0a0f] p-6 relative overflow-hidden">
      ...
      <div className="w-full max-w-sm relative z-10">
        <div className="flex flex-col items-center mb-8">
          ...
          <p className="text-sm text-slate-500 mt-1">Inizia a costruire i tuoi agenti AI</p>
        </div>

        <form action={action} className="space-y-4 rounded-2xl bg-white/[0.03] border border-white/[0.06] p-6">
          <div className="space-y-1.5">
            <label className="text-xs text-slate-400">Nome <span className="text-slate-600">(opzionale)</span></label>
            <input
              name="name"
              type="text"
              autoComplete="name"
          ...
          <div className="space-y-1.5">
            <label className="text-xs text-slate-400">Password <span className="text-slate-600">(min. 8 caratteri)</span></label>
            <input
              name="password"
              type="password"
              required
          ...
          >
            {pending ? <Loader2 size={16} className="animate-spin" /> : 'Registrati'}
          </button>
        </form>

        <p className="text-center text-xs text-slate-500 mt-4">
          Hai già un account?{' '}
          <Link href="/auth/sign-in" className="text-violet-400 hover:text-violet-300 font-medium">
            Accedi
          </Link>
        </p>
      </div>
